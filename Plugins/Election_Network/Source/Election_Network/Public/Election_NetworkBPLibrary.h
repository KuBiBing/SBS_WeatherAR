...
#include "Election_NetworkBPLibrary.generated.h"
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Election_MBCCNI_Socket")
	...
	UFUNCTION(BlueprintCallable, Category = "Election_MBCCNI_Socket") // use meta to set first default param to 0
		bool SendData(int32 ConnectionId, TArray<uint8> DataToSend);

	UFUNCTION(BlueprintCallable, Category = "Election_MBCCNI_Socket") // use meta to set first default param to 0
		bool SendDataString(int32 ConnectionId, FString sMsg);
	...
	*/
	...
	void ExecuteOnConnected(int32 WorkerId, TWeakObjectPtr<AElection_NetworkBPLibrary> thisObj);

	//UFUNCTION(Category = "Socket")
	void ExecuteOnDisconnected(int32 WorkerId, TWeakObjectPtr<AElection_NetworkBPLibrary> thisObj);

	//UFUNCTION(Category = "Socket")
	void ExecuteOnMessageReceived(int32 ConnectionId, TWeakObjectPtr<AElection_NetworkBPLibrary> thisObj);
	...
	UFUNCTION(BlueprintPure, meta = (DisplayName = "Append Bytes", CommutativeAssociativeBinaryOperator = "true"), Category = "Election_MBCCNI_Socket")
		static TArray<uint8> Concat_BytesBytes(TArray<uint8> A, TArray<uint8> B);

	/** Converts an integer to an array of bytes */
	UFUNCTION(BlueprintPure, meta = (DisplayName = "Int To Bytes", CompactNodeTitle = "->", Keywords = "cast convert", BlueprintAutocast), Category = "Election_MBCCNI_Socket")
		static TArray<uint8> Conv_IntToBytes(int32 InInt);

	/** Converts a string to an array of bytes */
	UFUNCTION(BlueprintPure, meta = (DisplayName = "String To Bytes", CompactNodeTitle = "->", Keywords = "cast convert", BlueprintAutocast), Category = "Election_MBCCNI_Socket")
		static TArray<uint8> Conv_StringToBytes(const FString& InStr);

	/** Converts a float to an array of bytes */
	UFUNCTION(BlueprintPure, meta = (DisplayName = "Float To Bytes", CompactNodeTitle = "->", Keywords = "cast convert", BlueprintAutocast), Category = "Election_MBCCNI_Socket")
		static TArray<uint8> Conv_FloatToBytes(float InFloat);

	/** Converts a byte to an array of bytes */
	UFUNCTION(BlueprintPure, meta = (DisplayName = "Byte To Bytes", CompactNodeTitle = "->", Keywords = "cast convert", BlueprintAutocast), Category = "Election_MBCCNI_Socket")
		static TArray<uint8> Conv_ByteToBytes(uint8 InByte);

	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Read Int", Keywords = "read int"), Category = "Election_MBCCNI_Socket")
		static int32 Message_ReadInt(UPARAM(ref) TArray<uint8>& Message);

	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Read Byte", Keywords = "read byte int8 uint8"), Category = "Election_MBCCNI_Socket")
		static uint8 Message_ReadByte(UPARAM(ref) TArray<uint8>& Message);

	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Read Bytes", Keywords = "read bytes"), Category = "Election_MBCCNI_Socket")
		static bool Message_ReadBytes(int32 NumBytes, UPARAM(ref) TArray<uint8>& Message, TArray<uint8>& ReturnArray);

	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Read Float", Keywords = "read float"), Category = "Election_MBCCNI_Socket")
		static float Message_ReadFloat(UPARAM(ref) TArray<uint8>& Message);

	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Read String", Keywords = "read string"), Category = "Election_MBCCNI_Socket")
	...
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Election_MBCCNI_Socket")
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Election_MBCCNI_Socket")
...
private:
	...
	FString ipAddress;
	int port;
	TWeakObjectPtr<AElection_NetworkBPLibrary> ThreadSpawnerActor;
	int32 id;
...
public:
	...
	FTcpSocketWorker(FString inIp, const int32 inPort, TWeakObjectPtr<AElection_NetworkBPLibrary> InOwner, int32 inId, int32 inRecvBufferSize, int32 inSendBufferSize, float inTimeBetweenTicks);
