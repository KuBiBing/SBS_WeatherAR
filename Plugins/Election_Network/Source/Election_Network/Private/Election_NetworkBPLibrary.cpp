...
#include "Election_NetworkBPLibrary.h"
...
AElection_NetworkBPLibrary::AElection_NetworkBPLibrary()
{
...
void AElection_NetworkBPLibrary::BeginPlay()
{
	Super::BeginPlay();
}

void AElection_NetworkBPLibrary::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void AElection_NetworkBPLibrary::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

void AElection_NetworkBPLibrary::Connect(const FString& ipAddress, int32 port, const FTcpSocketDisconnectDelegate& OnDisconnected, const FTcpSocketConnectDelegate& OnConnected,
	const FTcpSocketReceivedMessageDelegate& OnMessageReceived, int32& ConnectionId)
{
	...
	TWeakObjectPtr<AElection_NetworkBPLibrary> thisWeakObjPtr = TWeakObjectPtr<AElection_NetworkBPLibrary>(this);
...
void AElection_NetworkBPLibrary::Disconnect(int32 ConnectionId)
{
...
bool AElection_NetworkBPLibrary::SendData(int32 ConnectionId /*= 0*/, TArray<uint8> DataToSend)
{
...
bool AElection_NetworkBPLibrary::SendDataString(int32 ConnectionId /*= 0*/, FString sMsg)
{
	if (TcpWorkers.Contains(ConnectionId))
	{
		if (TcpWorkers[ConnectionId]->isConnected())
		{
			...
			TArray<uint8> btMsg = Conv_StringToBytes(sMsg);
			TcpWorkers[ConnectionId]->AddToOutbox(btMsg);
			return true;
		}
		else
		{
...
void AElection_NetworkBPLibrary::ExecuteOnMessageReceived(int32 ConnectionId, TWeakObjectPtr<AElection_NetworkBPLibrary> thisObj)
{
...
TArray<uint8> AElection_NetworkBPLibrary::Concat_BytesBytes(TArray<uint8> A, TArray<uint8> B)
{
...
TArray<uint8> AElection_NetworkBPLibrary::Conv_IntToBytes(int32 InInt)
{
	TArray<uint8> result;
	for (int i = 0; i < 4; i++)
	{
...
TArray<uint8> AElection_NetworkBPLibrary::Conv_StringToBytes(const FString& InStr)
{
...
TArray<uint8> AElection_NetworkBPLibrary::Conv_FloatToBytes(float InFloat)
{
	TArray<uint8> result;
...
TArray<uint8> AElection_NetworkBPLibrary::Conv_ByteToBytes(uint8 InByte)
{
	TArray<uint8> result{ InByte };
	return result;
}

int32 AElection_NetworkBPLibrary::Message_ReadInt(TArray<uint8>& Message)
{
	if (Message.Num() < 4)
	{
...
uint8 AElection_NetworkBPLibrary::Message_ReadByte(TArray<uint8>& Message)
{
	if (Message.Num() < 1)
	{
...
bool AElection_NetworkBPLibrary::Message_ReadBytes(int32 NumBytes, TArray<uint8>& Message, TArray<uint8>& returnArray)
{
...
float AElection_NetworkBPLibrary::Message_ReadFloat(TArray<uint8>& Message)
{
	if (Message.Num() < 4)
	{
...
FString AElection_NetworkBPLibrary::Message_ReadString(TArray<uint8>& Message, int32 BytesLength)
{
	if (BytesLength <= 0)
	{
		if (BytesLength < 0)
...
FString AElection_NetworkBPLibrary::Message_ReadElectionString(TArray<uint8>& Message)
{
	int nReceiveDataLength = Message.Num();

	if (nReceiveDataLength <= 0)
	{
		if (nReceiveDataLength < 0)
			PrintToConsole("Error in the ReadString node. BytesLength isn't a positive number.", true);
		return FString("");
	...
	if (nReceiveDataLength > 10)
	{
		TArray<uint8> strLength;
		strLength.Reserve(7);

		for (int i = 3; i < 10; i++)
		{
			strLength.Add(Message[i]);
		}

		std::string csLength(reinterpret_cast<const char*>(strLength.GetData()), strLength.Num());
		FString fsLength = FString(UTF8_TO_TCHAR(csLength.c_str()));

		int32 nPacketLength = FCString::Atoi(*fsLength);

		// 패킷 0x02 0x02 ^  이고 패킷길이가  수신받은 데이터 사이즈보다 크다면 파싱 작업을 한다
		if (Message[0] == 0x02 && Message[1] == 0x02 && Message[2] == '^' && nReceiveDataLength - 10 >= nPacketLength)
		{
			TArray<uint8> StringAsArray;
			StringAsArray.Reserve(nPacketLength);
		...
		else if (Message[0] != 0x02 || Message[1] != 0x02 && Message[2] != '^')
		{
			// 잘못된 패킷 정보이므로 버퍼에 있는 모든 데이터 날림
			for (int i = 0; i < nReceiveDataLength; i++)
			{
				Message.RemoveAt(0);
			}
			return FString("");
...
FString AElection_NetworkBPLibrary::Message_ReadFile(const FString& FullPath)
{
	...
	char* path = TCHAR_TO_ANSI(*FullPath);
	...
	while (!feof(fp))
	{
		...
		if (fgetws(tmpBuff, 4096, fp) == NULL)
			break;

		FString sLine(tmpBuff);
...
bool AElection_NetworkBPLibrary::isConnected(int32 ConnectionId)
{
	if (TcpWorkers.Contains(ConnectionId))
...
void AElection_NetworkBPLibrary::PrintToConsole(FString Str, bool Error)
{
...
void AElection_NetworkBPLibrary::ExecuteOnConnected(int32 WorkerId, TWeakObjectPtr<AElection_NetworkBPLibrary> thisObj)
{
	if (!thisObj.IsValid())
		return;
...
void AElection_NetworkBPLibrary::ExecuteOnDisconnected(int32 WorkerId, TWeakObjectPtr<AElection_NetworkBPLibrary> thisObj)
{
	if (!thisObj.IsValid())
		return;
...
FTcpSocketWorker::FTcpSocketWorker(FString inIp, const int32 inPort, TWeakObjectPtr<AElection_NetworkBPLibrary> InOwner, int32 inId, int32 inRecvBufferSize, int32 inSendBufferSize, float inTimeBetweenTicks)
	: ipAddress(inIp)
	, port(inPort)
...
{
	AsyncTask(ENamedThreads::GameThread, []() {	AElection_NetworkBPLibrary::PrintToConsole("Tcp socket thread was destroyed.", false); });
	Stop();
	if (Thread)
	{
...
{
	AsyncTask(ENamedThreads::GameThread, []() {	AElection_NetworkBPLibrary::PrintToConsole("Starting Tcp socket thread.", false); });
	...
	while (bRun)
	{
		...
		if (!bConnected)
		{
			...
			{
				AsyncTask(ENamedThreads::GameThread, []() { AElection_NetworkBPLibrary::PrintToConsole(FString::Printf(TEXT("Couldn't connect to server. TcpSocketConnection.cpp: line %d"), __LINE__), true); });
				bRun = false;
			}
			continue;
		}
		...
		if (!Socket)
		{
			AsyncTask(ENamedThreads::GameThread, []() { AElection_NetworkBPLibrary::PrintToConsole(FString::Printf(TEXT("Socket is null. TcpSocketConnection.cpp: line %d"), __LINE__), true); });
			bRun = false;
			continue;
		}
		...
		while (bRun)
		{
			...
			AsyncTask(ENamedThreads::GameThread, []() { AElection_NetworkBPLibrary::PrintToConsole("Pending data", false); });
			...
			if (!Socket->Recv(receivedData.GetData() + BytesReadTotal, PendingDataSize, BytesRead))
			{
				...
				AsyncTask(ENamedThreads::GameThread, []() {
					AElection_NetworkBPLibrary::PrintToConsole(FString::Printf(TEXT("In progress read failed. TcpSocketConnection.cpp: line %d"), __LINE__), true);
					});
				break;
			}
